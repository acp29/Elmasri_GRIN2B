...
allData %>% 
  ...
  mutate(cell = paste0(slice,cell))  -> Data
```
...
p1 <- Data %>%
    mutate(mutation_jittered = jitter((as.numeric(mutation)+(as.numeric(drug)-1)/2.5), 0.5),
           grouping=interaction(cell, mutation)) %>%
    mutate(mutation_drug = as.numeric(mutation)+(as.numeric(drug)-1)/2.5) %>%
    ggplot(aes(x=mutation, y=!!sym(resp), group=grouping, color=as.factor(mutation_drug))) + 
    geom_blank() +
    geom_line(aes(mutation_jittered), alpha=0.4, color="grey", size=0.66) +
    geom_point(aes(mutation_jittered), alpha=0.8, shape = 16, size=1.5) +
    scale_color_manual(values=c("grey","#7F7F7F","#FFB3E2","#FF60C3","#B2CDFF","#75AAFF")) +
    scale_fill_manual(values=c("grey","#7F7F7F","#FFB3E2","#FF60C3","#B2CDFF","#75AAFF")) +
    ylab(resp) +
    theme(axis.text.x = element_text(angle = 45, vjust=1, hjust=1),axis.line = element_line(colour="black"),
          panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
...
p2 <- Data %>% 
    ...
    mutate(ratio = `after`/`before`) %>%
    ggplot(aes(x=mutation, y=ratio, colour=mutation)) +
    geom_sina(alpha=0.6, shape=16, size=1.5, maxwidth=0.5) + 
