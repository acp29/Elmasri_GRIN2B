...
```{r}
Data <- read.delim("../data/n2b_mutant_tcn201.dat", header = TRUE)
...
attr(Data$drug,"contrasts") %>% 
  ...
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>%
  print() 
formula <- sprintf("log(%s) ~ mutation * drug + (1|cell)", resp)
...
infl <- hlm_influence(model, level="cell")
p4 <- infl %>% 
  mutate(influential = cooksd > 1.0) %>% 
  ggplot(aes(x=`cell`,y=cooksd, color=influential)) + 
  geom_segment(aes(x=`cell`, xend=`cell`, y=0, yend=cooksd)) + 
  geom_point() + 
...
Data %>% 
  ...
  as.data.frame() -> Data
set.seed(123456)
anovaBF(logresp ~ mutation * drug + cell, 
                 whichRandom = "cell", 
