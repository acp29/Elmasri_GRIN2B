---
title: "GluN2B mutant rescue in double KO (*grin2a* and *grin2b*): NMDA-EPSC charge"
output:
...
```{r}
Data$mutation <- factor(Data$mutation, levels=c("WT_2B","R540H","R696H","C456Y"))
...
p2 <- Data %>% 
    ...
    scale_color_manual(values=c("grey","#DB72FB","#FF61C3","#619CFF")) +
...
# ("WT_2B","R540H","R696H","C456Y")
WT_vs_mutant <- c(-3,1,1,1)/4
LOF_vs_GOF <- c(0,1,1,-2)/3
GOF        <- c(0,-1,1,0)/2

contr.orth <- cbind(WT_vs_mutant, LOF_vs_GOF, GOF)
...
emf(sprintf("../img/%s_%s.emf","n2b_dko_mutant_nmdar",resp), width=4, height=3.5)
emm %>%
    as.data.frame() %>%
...
p2 <- Data %>% 
    ...
    scale_color_manual(values=c("grey","#DB72FB","#FF61C3","#619CFF")) +
    scale_fill_manual(values=c("grey","#DB72FB","#FF61C3","#619CFF")) +
    ylab("ratio") +
    theme(axis.text.x = element_text(angle = 45, vjust=1, hjust=1), axis.line = element_line(colour="black"),
          panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
...
model_parameters(model, df_method = "kenward", exponentiate = TRUE, effects = "fixed") %>%  
  ...
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover")) %>%
  add_indent(c(4:6)) %>%                                    # add indentation to indicate source components
  print()
