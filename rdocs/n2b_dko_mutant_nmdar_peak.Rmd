---
title: "GluN2B mutant rescue in double KO (*grin2a* and *grin2b*): NMDA-EPSC peak"
output:
...
```{r}
allData <- read.delim("../data/n2b_dko_mutant_nmdar.dat", header = TRUE)
...
```{r}
Data$mutation <- factor(Data$mutation, levels=c("DKO","WT_2B","R540H","R696H","C456Y"))
...
p2 <- Data %>% 
    ...
    scale_color_manual(values=c("#00B04F","grey","#DB72FB","#FF61C3","#619CFF")) +
...
# ("DKO",WT_2B","R540H","R696H","C456Y")
DKO_vs_N2B <- c(-4,1,1,1,1)/5
WT_vs_mutant <- c(0,-3,1,1,1)/4
LOF_vs_GOF <- c(0,0,1,1,-2)/3
GOF        <- c(0,0,-1,1,0)/2

contr.orth <- cbind(DKO_vs_N2B,WT_vs_mutant, LOF_vs_GOF, GOF)
...
emf(sprintf("../img/%s_%s.emf","n2b_dko_mutant_nmdar",resp), width=5, height=3.5)
emm %>%
    as.data.frame() %>%
...
p1 <- Data %>%
    ...
    ylab(resp) +
    geom_hline(yintercept = emm_df$response[6], linetype = 'dashed', col = "#00B04F") +
    theme(axis.text.x = element_text(angle = 45, vjust=1, hjust=1),axis.line = element_line(colour="black"),
          panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
          panel.border = element_blank(),
...
p2 <- Data %>% 
    ...
    scale_color_manual(values=c("#00B04F","grey","#DB72FB","#FF61C3","#619CFF")) +
    scale_fill_manual(values=c("#00B04F","grey","#DB72FB","#FF61C3","#619CFF")) +
    ylab("ratio") +
    geom_hline(yintercept = emm.transfection_df$ratio[1],linetype = 'dashed', col = "#00B04F") +
    theme(axis.text.x = element_text(angle = 45, vjust=1, hjust=1), axis.line = element_line(colour="black"),
          panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
          panel.border = element_blank(),
...
# Dunnett's stepdown adjustment to control FWER on p-values (using multcomp package)
